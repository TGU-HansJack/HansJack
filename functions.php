...
function parseEmbedShortcodeNamedArgs(string $args): array
{
    $decoded = trim(html_entity_decode($args, ENT_QUOTES | ENT_HTML5, 'UTF-8'));
    if ($decoded === '') {
        return [];
    ...
    $matched = preg_match_all(
        '/([A-Za-z][A-Za-z0-9_-]*)\s*=\s*(?:"([^"]*)"|\'([^\']*)\'|([^\s"\']+))/u',
        $decoded,
        $matches,
        PREG_SET_ORDER
    );
    if (!is_int($matched) || $matched <= 0 || !is_array($matches)) {
        return $pairs;
    }
    ...
    foreach ($matches as $entry) {
        ...
        $key = strtolower(trim((string) ($entry[1] ?? '')));
        if ($key === '') {
            continue;
        }

        $value = '';
        if (array_key_exists(2, $entry) && $entry[2] !== '') {
            $value = (string) $entry[2];
        } elseif (array_key_exists(3, $entry) && $entry[3] !== '') {
            $value = (string) $entry[3];
        } elseif (array_key_exists(4, $entry)) {
            $value = (string) $entry[4];
        }
        ...
        <div class="comment-author" itemprop="creator">  
            ...
            <div class="comment-author-meta">
                ...
                <?php if (strpos($commentClass, 'comment-by-author') !== false): ?>
                    <span class="comment-author-badge" role="img" aria-label="<?php _e('笔者'); ?>" title="<?php _e('笔者'); ?>" data-comment-author-tip="<?php _e('笔者'); ?>">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-signature-icon lucide-signature" aria-hidden="true"><path d="m21 17-2.156-1.868A.5.5 0 0 0 18 15.5v.5a1 1 0 0 1-1 1h-2a1 1 0 0 1-1-1c0-2.545-3.991-3.97-8.5-4a1 1 0 0 0 0 5c4.153 0 4.745-11.295 5.708-13.5a2.5 2.5 0 1 1 3.31 3.284"/><path d="M3 21h18"/></svg>
                    </span>
                <?php endif; ?>
                <?php if ($commentUrl !== ''): ?>
                    <a
